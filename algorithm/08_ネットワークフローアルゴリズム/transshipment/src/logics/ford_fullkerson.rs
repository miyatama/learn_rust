...
fn find_argumenting_path(graph: &Graph) -> Option<HashMap<usize, VertexInfo>> {
    ...
    queue.push_back(source_vertex_index);
    arrival_cost.insert(source_vertex_index, 0i64);

    while let Some(from_vertex_index) = queue.pop_back() {
...
fn get_arrival_cost(costs: &HashMap<usize, i64>, index: usize) -> i64 {
    *costs.get(&index).unwrap_or(&i64::MAX)
}
