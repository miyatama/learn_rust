...
<details><summary>結果生成パス</summary>
...
stateDiagram-v2
  ...
  state "s" as v_0
  state "t" as v_99999
  state "生産拠点A" as v_1000
  state "生産拠点B" as v_1001
  state "小売店A" as v_3000
  state "小売店B" as v_3001
  v_0 --> v_1000
  v_0 --> v_1001
  v_1000 --> v_3000
  v_1000 --> v_3001
...
stateDiagram-v2
  ...
  state "小売店A" as v_3000
  state "小売店B" as v_3001
  v_1001 --> v_3000: 280/280
  v_1000 --> v_3001: 200/200
  v_3001 --> v_99999: 300/300
  v_1001 --> v_3001: 100/350
  v_0 --> v_1001: 380/800
  v_3000 --> v_99999: 300/300
  v_1000 --> v_3000: 20/200
  v_0 --> v_1000: 220/600
```
...
問題: 供給基地$`s_i`$から需要基地$`t_j`$への最小コストを求める
...
+ n個の需要基地$`t_j`$
  ...
  + $`1 \leqq j \leqq n`$
+ w個の倉庫基地
  + 最大受け取り能力: $`max(w_k)`$ [ユニット]
  + 積み替えコスト: $`transship(w_k)`$ [コスト / ユニット]
  + $`1 \leqq k \leqq w`$
+ 固定コスト
  + $`s_i`$から$`t_j`$への出荷: $`d(i, j)`$ [コスト / ユニット]
  + $`s_i`$から$`w_k`$への輸送: $`ts(i k)`$ [コスト / ユニット]
  + $`w_k`$から$`t_j`$への輸送: $`ts(k, j)`$ [コスト / ユニット]
  + $`w_k1`$から$`w_k2`$への輸送: $`ts(k1, k2)`$ [コスト / ユニット]
+ 全体コスト: $`TC = TSC + TTC`$
  + TSC: 全輸送コスト
    + $`\sum_i \sum_j d(i, j) * f(i, j)`$
  + TSS: 全積み替えコスト
    + $`\sum_i \sum_k ts(i, k) * f(i, k) + \sum_k\sum_j ts(k, j) * f(k, j) + `$
+ 制約
  + 最終的に倉庫$`w_k`$を通るフローの合計は0となる(倉庫にはなにも残らない)
  + 供給能力: $`sup(s_i)`$、需要能力: $`dem(t_j)`$はともに0より大きい
  + 輸送・積み替えコストは0以上
  ...
  + $`G = (V, E)`$
  + V: $`個数 = n + m + 2 * w + 2`$
    + m個の供給基地$`s_i`$: 番号$`i`$の節点に該当
    + w個の倉庫基地$`w_k`$: 2つの節点$`m + 2 * k - 1`$と$`m + 2 * k`$に該当
    + n個の需要基地$`t_j`$: $`m + 2 * w + j`$の節点に該当
    + ソース: $`0`$の節点に該当
    + シンク: $`n + m + 2 * w + 1`$の節点に該当
  + E: $`個数 = (w + 1) * (m + n) + m * n + w`$
    + ソース -> m個の供給基地
      + コスト: 0
      + 容量: $`sup(s_i)`$
    + m個の供給基地 -> n個の需要基地(特定の需要基地に向けてかかるコスト算出ルート)
      + コスト: $`d(i, j)`$
      + 容量: $`\infty`$
    + n個の需要基地 -> シンク
      + コスト: 0
      + 容量: $`dem(t_j)`$
    + w個の倉庫節点 -> w個の倉庫節点(積み替えコスト算出用のルート)
      + コスト: $`max(w_k)`$
      + 容量: $`transship(w_k)`$
    + m個の供給基地 -> w個の倉庫節点
      + コスト: $`ts(i, k)`$
...
stateDiagram-v2
  direction LR;
  state "生産拠点A(600)" as v_1000
  state "生産拠点B(800)" as v_1100
  state "倉庫A" as v_2000
  state "倉庫B" as v_2500
  state "小売店A(300)" as v_3000
  state "小売店B(300)" as v_3100
  v_1000 --> v_2000
  v_1000 --> v_2500
  v_1100 --> v_2000
  v_1100 --> v_2500
...
stateDiagram-v2
  ...
  state "s" as v_0
  state "生産拠点A" as v_1000
  state "生産拠点B" as v_1100
  state "倉庫A" as v_2000
  state "倉庫B" as v_2500
  state "小売店A" as v_3000
  state "小売店B" as v_3100
  state "t" as v_99999
  v_0 --> v_1000
  v_1000 --> v_3000
  v_1000 --> v_3100
