...
fn quick_sort(src_list: Vec<u64>) -> Vec<u64> {
    quick_sort_main(src_list.clone(), get_pivot_index(src_list))
}

fn quick_sort_main(src_list: Vec<u64>, pivot_index: usize) -> Vec<u64> {
    let len = src_list.len();
    if len <= 1 {
        return src_list;
    }
    if len < 100 {
        return heap_sort(src_list);
    }

    let (l1, l2) = partition(src_list, pivot_index);
    let mut new_l1 = quick_sort(l1);
    let new_l2 = quick_sort(l2);
    new_l1.extend(new_l2);
    new_l1
}

fn partition(list: Vec<u64>, pivot_index: usize) -> (Vec<u64>, Vec<u64>) {
    let len = list.len();
    match pivot_index {
        i if i < 1 => (list, vec![]),
        i if i > len => (list, vec![]),
        _ => {
            let base_value = list[pivot_index];
            (
                list.iter()
                    .filter(|val| **val <= base_value)
                    .map(|val| *val)
                    .collect::<Vec<u64>>(),
                list.iter()
                    .filter(|val| **val > base_value)
...
fn get_pivot_index(list: Vec<u64>) -> usize {
    list.len() / 2
}
