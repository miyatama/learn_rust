use super::alpha_beta::{self, CellValue};
    ...
    loop {
        ...
        match input_to_index(prompt, &board) {
            Some(index) => {
                ...
                alpha_beta::print_board(&board);
                if alpha_beta::is_player_win(&board, CellValue::Player) {
                    println!("おめでとうございます！完全勝利です！");
                    break;
                }
                if alpha_beta::none_cell_not_exists(&board) {
                    println!("引き分けですかね。対戦ありがとうございました。");
                    break;
                }

                println!("考え中....");
                let enemy_move = alpha_beta::best_move(&board);
                ...
                alpha_beta::print_board(&board);
                if alpha_beta::is_player_win(&board, CellValue::Enemy) {
